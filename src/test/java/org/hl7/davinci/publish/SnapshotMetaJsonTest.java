...
class SnapshotMetaJsonTest {
	...
	void roundTripsAndToleratesUnknownFieldsOnReplay() throws Exception {
		...
		PublishService.SnapshotMeta meta = new PublishService.SnapshotMeta(
				...
				List.of(
						new PublishService.FileMeta("Organization", 3, 456, "snap-1"),
						new PublishService.FileMeta("Location", 1, 100, "snap-1")));
		...
		String jsonWithUnknownFields =
				"""
				{
				  ...
				  "bucketVersion": 3,
				  "files": [
				    {"type": "Organization", "count": 3, "fileSize": 456, "snapshotId": "snap-1", "bucket": 3},
				    {"type": "Location", "count": 1, "fileSize": 100, "snapshotId": "snap-1", "bucket": 3}
				  ]
				}
				""";
