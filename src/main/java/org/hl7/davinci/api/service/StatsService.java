...
public class StatsService {
	...
	public JobStatsResponse computeStats(String jobId) {
		...
		long totalBuildMs = manifests.stream().mapToLong(ManifestRecord::getBuildDurationMs).sum();
		...
		int completedRuns = (int) runs.stream().filter(r -> r.getStatus() == RunStatus.COMPLETED).count();
		int erroredRuns = (int) runs.stream().filter(r -> r.getStatus() == RunStatus.ERROR).count();
