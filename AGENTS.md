...
- `CrawlResource` - current aggregate keyed `serverKey|Type/id`, server-scoped (shared by all jobs targeting that server; cleared only when the last such job is deleted). Body stored as raw gzip via `ResourceJsonCodec`. Implements `Persistable<String>` so diff-classified inserts skip the per-row exists-SELECT.
