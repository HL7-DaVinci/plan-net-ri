...
- `CrawlJob` - schedule (Spring cron; blank = manual-only, skipped by the scheduler), `enabled` (pause = enabled=false), `running` (display flag; can go stale after restart)
...
- `ManifestRecord` - one retained snapshot: DB row + `storageDir` pointing at NDJSON files on disk (`api.storage-path`)
- `CrawlResource` - current aggregate state keyed `serverKey|Type/id`. Server-scoped, NOT job-scoped: shared by all jobs targeting that server, so never cascade-delete it with a job. Bodies are stored compressed via `ResourceJsonCodec` (`gz:` + Base64(gzip); values without the prefix are legacy plaintext and must keep decoding in place).
...
1. Diff: `DiffUtil.computeDiff` classifies fetched resources against the `CrawlResource` version index (added when key absent; unchanged when versionId OR lastUpdated matches; updated otherwise).
2. Persist: `CrawlPersistenceService.persistFullSnapshot` and `persistIncremental` both upsert only changed rows and delete missing/resolved keys (never wipe-and-rewrite; that bloats H2's MVStore). `PersistCounts` carries added/updated/deleted plus the post-run `total`.
3. Manifest: when all servers complete (and the run was not cancelled), `ManifestService.createManifest` writes the NDJSON snapshot and prunes to `api.retention-per-job`. Runs and steps have no retention (deliberate, deferred).
